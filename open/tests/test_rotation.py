...
def test_build_read_timeout_default_and_explicit():
    ...
    assert build_read_timeout().read == 90.0
...
def test_once_alive_pinned_forever_across_ttl(tmp_path, monkeypatch):
    """Answer once -> pinned: later failed probes (even after TTL expiry) keep
    it alive, and pinned models are not probed at all."""
    import json
    import time
    ...
    # TTL long expired, and the probe now 'fails' for everything:
    ...
    second = rotation.check_zen_model_health(["x-preview-f-free"])
    assert second["x-preview-f-free"] is True  # pin wins
    assert probed == ["x-preview-f-free"]      # never re-probed

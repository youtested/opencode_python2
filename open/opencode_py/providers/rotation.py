...
def build_read_timeout(read_seconds: float | None = None, model: str = "") -> httpx.Timeout:
    ...
    short read timeout kills mid-conversation turns. The 90s default gives
    the model room without letting one dead connection stall a turn in
    minutes of total silence; users can raise/lower it in Settings.
    ...
    """
    read = float(read_seconds) if read_seconds else 90.0
        ...
        for index, lane in enumerate(lanes):
            ...
            try:
                ...
                if not had_output:
                    ...
                    # Rotate the session identity so the retry draws a fresh
                    # lane instead of re-rolling the same dead one 50 times.
                    self._rotate_lane(provider)
...
def _probe_headers() -> dict:
    ...
    # model dead that answers fine in real chat.
    key = os.environ.get("OPENCODE_API_KEY")
    headers["Authorization"] = f"Bearer {key or 'public'}"
    headers["x-opencode-session"] = headers["x-opencode-request"]
    return headers
...
def _probe_zen_model(model_id: str) -> bool:
    ...
    200 = alive; 429 = alive but throttled (don't hide a working model during
    a burst); anything else (401 no-key / 400 / 503 dead upstream) = not usable.
    """
    try:
        import httpx
...
def check_zen_model_health(model_ids: list[str], ttl_seconds: int = HEALTH_TTL_SECONDS) -> dict[str, bool]:
    ...
    Pinning: a model that answered correctly ONCE is recorded in the cache's
    ``pinned`` map and reported alive FOREVER — no more 30-minute windows
    hiding a working model after one transient blip. A pin only disappears
    when the model itself vanishes from the passed-in list (i.e. the Zen
    provider removed it from the catalog); unpinned models are re-probed when
    the TTL expires and get a fresh second chance.
    ...
    known = set(model_ids)
    dropped = [mid for mid in pinned if mid not in known]
    for mid in dropped:
    ...
    for mid in model_ids:
        if pinned.get(mid):
            results[mid] = True  # answered once before: trusted permanently
        elif not fresh or mid not in cached:
            to_probe.append(mid)  # unknown or stale window: probe now
        else:
    ...
    if to_probe:
        ...
        # fail-open: probes all failed -> don't trust them, keep everything visible
        if to_probe and not any(probed.values()):
            probed = {mid: True for mid in to_probe}

        for mid, ok in probed.items():
            results[mid] = ok
            if ok and mid not in pinned:
                pinned[mid] = time.time()  # responded once -> pinned forever

    merged = dict(results)
    try:
        cache_path.parent.mkdir(parents=True, exist_ok=True)
        cache_path.write_text(
            json.dumps({"ts": time.time(), "health": merged, "pinned": pinned}),
            encoding="utf-8",
        )
    except OSError:
        pass
...
def fetch_zen_models(cache_file=None, ttl_hours: int = 1) -> list[dict]:
    ...
    Primary source: the models.dev catalog (names like "Ox Alpha Free
    (Unlimited)", real context limits, $0 pricing → FREE badge). Models added
    or removed upstream show up/vanish on their own via the cached catalog +
    hourly background refresh. Fallbacks: Zen's own /models endpoint, then the
    bundled free-model list.
    """
    models = _zen_models_from_catalog(fetch_catalog())
    if models:
        # Show only the free models that actually respond right now; paid ones
        # just need a key, so they aren't probed/filtered.
        ...
        if free_ids:
            ...
            models = [
                m
                for m in models
                if (not m["free"]) or (m["id"] in alive)
            ]
